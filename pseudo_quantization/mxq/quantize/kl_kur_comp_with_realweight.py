...
def select_quantile_threshold(x: torch.Tensor, q: float):
    if x.ndim != 1:
        x = x.reshape(-1)
    if x.numel() == 0:
        raise ValueError("select_quantile_threshold received an empty tensor")
    q = min(max(q, 0.0), 1.0)
    rank = int(q * (x.numel() - 1)) + 1
    return x.kthvalue(rank).values
...
def main():
    ...
    # only process first 10 files for quick testing
    files = files[:10]
    x_axis = list(range(min_j, min_j + len(files)))
    ...
    for name in files:
        ...
        tensor_value = torch.tensor(x, device = device)
        ...
        good_threshold = select_quantile_threshold(mse_per_l1_group, 0.1)
        bad_threshold = select_quantile_threshold(mse_per_l1_group, 0.9)
    ...
    plot_metric(
        ...
        "Cross Entropy",
        "dump/cross_entropy_elem_comparison_realinput.png",
    )
    plot_metric(
        x_axis,
        good_acc["ce_hist"],
        ...
        "Cross Entropy",
        "dump/cross_entropy_hist_comparison_realinput.png",
    )
    plot_metric(
        x_axis,
        ...
        "Cross Entropy",
        "dump/cross_entropy_elem_sorted_comparison_realinput.png",
    )
    plot_metric(
        x_axis,
        ...
        "dump/wasserstein_comparison_realinput.png",
    )
    plot_metric(
        x_axis,
        ...
        "Mutual Information",
        "dump/mutual_information_comparison_realinput.png",
    )
    plot_metric(
        x_axis,
        good_acc["kl_elem"],
        ...
        "KL Divergence",
        "dump/kl_divergence_elem_comparison_realinput.png",
    )
    plot_metric(
        x_axis,
        ...
        "KL Divergence",
        "dump/kl_divergence_elem_sorted_comparison_realinput.png",
    )
    plot_metric(
        x_axis,
        good_acc["kl_hist"],
        ...
        "KL Divergence",
        "dump/kl_divergence_hist_comparison_realinput.png",
    )
    plot_metric(
        x_axis,
        ...
        "Kurtosis Comparison",
        "Excess Kurtosis",
        "dump/kurtosis_avg_comparison_realinput.png",
    )
    ...
    plot_histogram(
        ...
        "Absolute Value",
        "dump/good_bad_groups_data_histogram_comparison_realinput.png",
    )
    plot_histogram(
        ...
        "Excess Kurtosis",
        "dump/kurtosis_distribution_comparison_realinput.png",
    )
