// =============================================================
// PE Tile for MXFP (Verilog-2001 compatible FIX v2)
...
(
    ...
    // [FIX] Moved all integer declarations for loops here
    integer si, i, j;
    ...
    always @(posedge clk or negedge rst_n) begin
      // integer i; // [FIX] Removed from here
      if (!rst_n) begin
        v1 <= 1'b0;
    ...
    always @(posedge clk or negedge rst_n) begin
      // integer j; // [FIX] Removed from here
      if (!rst_n) begin
...
`else
    ...
    reg [31:0] psum_out_fp32_r;
    always @(posedge clk or negedge rst_n) begin
        if (!rst_n) psum_out_fp32_r <= 32'h0;
        else if (v8) psum_out_fp32_r <= sum_s8;
    end
    
    assign psum_out_fp32 = psum_out_fp32_r;
    assign out_valid     = v8;
`endif
endmodule