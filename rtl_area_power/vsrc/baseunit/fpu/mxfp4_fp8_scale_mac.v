...
// FP16 乘法器模块
module fp16_mul (
    input  wire [15:0] a,         // FP16 输入 a
    input  wire [7:0]  b,         // FP8 输入 b
    output wire [15:0] c,         // FP16 乘法结果
    output wire        error      // 错误标志
);
    ...
    wire [10:0] a_mant;
    wire [2:0] b_mant;            // FP8 尾数
    wire [12:0] mant_product;     // 尾数乘积
    wire [4:0] exp_sum;
    ...
    assign b_exp  = b[6:2];
    assign a_mant = {1'b1, a[9:0]}; // 隐藏位
    assign b_mant = {1'b1, b[1:0]}; // 隐藏位

    // 符号计算
    assign c_sign = a_sign ^ b_sign;

    // 指数相加
    assign exp_sum = a_exp + b_exp - 5'd15;

    // 尾数相乘
    assign mant_product = a_mant * b_mant;

    // 溢出/下溢检测
    assign overflow = (exp_sum > 5'd30);  // 超过 FP16 指数范围
    assign underflow = (exp_sum < 5'd1);  // 小于 FP16 最小指数

    // 规范化输出
    assign c_exp = overflow ? 5'd31 : (underflow ? 5'd0 : exp_sum);
    assign c = {c_sign, c_exp, mant_product[12:3]}; // 截断尾数
    assign error = overflow | underflow;

endmodule

// FP8 乘法器模块（合成 Scaling Factor）
module fp8_mul (
    input  wire [7:0] a,         // FP8 输入 a
    input  wire [7:0] b,         // FP8 输入 b
    output wire [15:0] c,        // 合成结果 (FP16)
    output wire        error     // 错误标志
);

    wire a_sign, b_sign, c_sign;
    wire [4:0] a_exp, b_exp, c_exp;
    wire [2:0] a_mant, b_mant;
    wire [5:0] mant_product;
    wire [4:0] exp_sum;
    wire overflow, underflow;
    ...
    // 溢出/下溢检测
    assign overflow = (exp_sum > 5'd30);
    assign underflow = (exp_sum < 5'd1);

    // 规范化输出
    assign c_exp = overflow ? 5'd31 : (underflow ? 5'd0 : exp_sum);
    assign c = {c_sign, c_exp, mant_product[5:0]};
