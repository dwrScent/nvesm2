module mxfp4_mac (
    ...
    wire [15:0] scaled_result;    // 应用缩放后的结果
...
// Scaling Factor 应用模块
module scale_applier (
    input  wire [15:0] c_in,      // 输入累加结果 (FP16)
    input  wire [7:0]  scale_a,  // Scaling factor A (E8M0)
    input  wire [7:0]  scale_b,  // Scaling factor B (E8M0)
    output wire [15:0] c_out     // 缩放后的结果 (FP16)
);

    wire [4:0] exponent_in;
    wire [4:0] exponent_out;
    wire [10:0] mantissa;
    wire sign;

    // 提取输入的符号、指数、尾数
    assign sign       = c_in[15];
    assign exponent_in = c_in[14:10];
    assign mantissa   = c_in[9:0];

    // 应用 Scaling Factor
    assign exponent_out = exponent_in + scale_a + scale_b;

    // 重新组合结果
    assign c_out = {sign, exponent_out, mantissa};
