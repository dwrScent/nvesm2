...
from accelerator.src.utils.utils import ceil_a_by_b, log2, lookup_pandas_dataframe
from accelerator.src.simulator.stats import Stats
from accelerator.src.simulator.loop_stack import LoopStack
from accelerator.src.optimizer.optimizer import optimize_for_order, get_stats_fast
from accelerator.src.simulator.accelerator import Accelerator
from accelerator.src.simulator.energy import EnergyTuple

from accelerator.sram.cacti_sweep import CactiSweep
import os
import pandas

from accelerator.src.tensorOps.cnn import Convolution, MatMul
...
class Simulator(object):
    ...
    def __init__(self, config_file='conf.ini', verbose=False, energy_costs=None, core_csv_path=None):

        # custom energy cost
        self.energy_costs = energy_costs

        # custom core PPA CSV path (per accelerator scheme)
        self.core_csv_path = core_csv_path
    ...
    def get_area(self):
        ...
        # core_csv = os.path.join('./configs/ppa', 'systolic_array_synth.csv')
        default_core_csv = os.path.join('./configs/ppa', 'systolic_array_synth.csv')
        core_csv = self.core_csv_path if self.core_csv_path is not None else default_core_csv
